...
class DbConnection:
    ...
    def get_users_db_connection_instance():
        return psycopg2.connect(host=os.environ.get('DATABASE_HOST'), database=os.environ.get('DATABASE_NAME_USER'),
                                user=os.environ.get('DATABASE_USER'), password=os.environ.get('DATABASE_PASSWORD'))
    ...
    def get_short_url(url, cur=None):
        try:
            ...
            if not cur:
                ...
                cur = conn.cursor()
            cur.execute("""INSERT INTO url_shortner (url, clicks, date_created) VALUES (%s, %s, %s) 

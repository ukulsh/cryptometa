...
from project.api.queries import (
    ...
    select_wallet_reconciliation_query,
)
...
RECONCILIATION_DOWNLOAD_HEADERS = [
    "Order ID",
    "Raised Date",
    "AWB",
    "Courier",
...
class WalletDeductions(Resource):
        ...
        try:
            ...
            if auth_data.get("user_group") in ("super-admin", "client", "multi-vendor"):
                ...
                if filters:
                    ...
                    if "time" in filters:
                        ...
                        query_to_execute = query_to_execute.replace(
                            ...
                            "AND aa.status_time between '%s' and '%s'" % (filter_date_start, filter_date_end),
                        )
                if auth_data["user_group"] == "client":
                    query_to_execute = query_to_execute.replace(
                        "__CLIENT_FILTER__", "AND dd.client_prefix = '%s'" % client_prefix
                    )
                if auth_data["user_group"] == "multi-vendor":
                    ...
                    query_to_execute = query_to_execute.replace(
                        "__MV_CLIENT_FILTER__", "AND dd.client_prefix in %s" % str(tuple(vendor_list))
                    )
                ...
                query_to_execute = (
                    ...
                    .replace("__COURIER_FILTER__", "")
                    .replace("__DATE_TIME_FILTER__", "")
                )
                ...
                cur.execute(
                    "SELECT COALESCE(sum(tot_amount+total_charge), 0), count(*) FROM ("
                    + query_to_execute.replace("__PAGINATION__", "")
                    + ") xx"
                )
    ...
    def get(self, resp):
        try:
            ...
            if auth_data["user_group"] == "client":
                query_to_run_courier = query_to_run_courier.replace(
                    "__CLIENT_FILTER__", "AND dd.client_prefix='%s'" % auth_data["client_prefix"]
                )
                ...
                if all_vendors:
                    query_to_run_client = query_to_run_client.replace(
                        "__CLIENT_FILTER__", "AND cc.client_prefix in %s" % str(tuple(all_vendors))
                    )
                    query_to_run_courier = query_to_run_courier.replace(
                        "__CLIENT_FILTER__", "AND dd.client_prefix in %s" % str(tuple(all_vendors))
                    )
                else:
            ...
            if auth_data.get("user_group") in ("super-admin", "client", "multi-vendor"):
                ...
                if filters:
                    ...
                    if "recharge_time" in filters:
                        ...
                        query_to_execute = query_to_execute.replace(
                            "__DATE_TIME_FILTER__",
                            "AND recharge_time between '%s' and '%s'" % (filter_date_start, filter_date_end),
                        )
                if auth_data["user_group"] == "client":
                    query_to_execute = query_to_execute.replace(
                ...
                if auth_data["user_group"] == "multi-vendor":
                    ...
                    vendor_list = cur.fetchone()[0]
                    query_to_execute = query_to_execute.replace(
                        "__MV_CLIENT_FILTER__", "AND client_prefix in %s" % str(tuple(vendor_list))
                    )
            ...
            if auth_data["user_group"] in ("super-admin", "multi-vendor"):
                ...
                if all_vendors:
                    query_to_run = query_to_run.replace(
                        "__CLIENT_FILTER__", "WHERE client_prefix in %s" % str(tuple(all_vendors))
                    )
                else:
        ...
        try:
            ...
            if unique_id:
                ...
                if auth_data.get("user_group") == "client":
                    query_to_execute = query_to_execute.replace(
                        "__CLIENT_FILTER__", "and xx.client_prefix='%s'" % client_prefix
                    )
                else:
            ...
            if filters:
                ...
                if "remittance_date" in filters:
                    ...
                    query_to_execute = query_to_execute.replace(
                        ...
                        "AND remittance_date between '%s' and '%s'" % (filter_date_start, filter_date_end),
                    )
            ...
            if search_key:
                query_to_execute = query_to_execute.replace(
                    ...
                    "AND transaction_id ilike '%__SEARCH_KEY__%'".replace("__SEARCH_KEY__", search_key),
                )

            query_to_execute = (
                query_to_execute.replace("__SEARCH_KEY_FILTER__", "")
                .replace("__CLIENT_FILTER__", "")
                .replace("__REMITTANCE_DATE_FILTER__", "")
            )
            cur.execute(query_to_execute.replace("__PAGINATION__", ""))
            total_count = cur.rowcount
            query_to_execute = query_to_execute.replace(
    ...
    def get(self, resp):
        try:
            ...
            query_to_run_status = """SELECT status, count(*) FROM cod_remittance
                                                    ...
                                                    ORDER BY status"""
            if auth_data["user_group"] == "multi-vendor":
                query_to_run_status = query_to_run_status.replace(
                    "__CLIENT_FILTER__", "WHERE client_prefix in %s" % str(tuple(all_vendors))
                )
            elif auth_data["user_group"] == "client":
                query_to_run_status = query_to_run_status.replace(
                    "__CLIENT_FILTER__", "WHERE client_prefix = '%s'" % auth_data["client_prefix"]
                )
            else:
            ...
            if auth_data.get("user_group") in ("super-admin", "client", "multi-vendor"):
                ...
                if filters:
                    ...
                    if "time" in filters:
                        ...
                        query_to_execute = query_to_execute.replace(
                            "__DATE_TIME_FILTER__",
                            "AND aa.raised_date between '%s' and '%s'" % (filter_date_start, filter_date_end),
                        )
                ...
                if auth_data["user_group"] == "client":
                    query_to_execute = query_to_execute.replace(
                        "__CLIENT_FILTER__", "AND ee.client_prefix = '%s'" % client_prefix
                    )
                if auth_data["user_group"] == "multi-vendor":
                    cur.execute("SELECT vendor_list FROM multi_vendor WHERE client_prefix='%s';" % client_prefix)
                    vendor_list = cur.fetchone()[0]
                    query_to_execute = query_to_execute.replace(
                        "__MV_CLIENT_FILTER__", "AND ee.client_prefix in %s" % str(tuple(vendor_list))
                    )
...
def get_courier_charges(resp):
    ...
    try:
        ...
        cur.execute(
            ...
            "WHERE client_prefix=%s;".replace("__ZONE__", zone_column_mapping[delivery_zone]).replace(
                "__ZONE_STEP__", zone_step_charge_column_mapping[delivery_zone]
            ),
            cost_select_tuple,
        )
        ...
        cur.execute(
            ...
            "LEFT JOIN master_couriers bb on aa.courier_id=bb.id "
            "__COURIER_FILTER__;".replace("__ZONE__", zone_column_mapping[delivery_zone])
            .replace("__ZONE_STEP__", zone_step_charge_column_mapping[delivery_zone])
            .replace("__COURIER_FILTER__", courier_name_filter)
        )
        charge_rate_values = cur.fetchall()
        for crv in charge_rate_values:
            cost_list.append(crv)
        ...
        for cost_obj in cost_list:
            cur.execute(
                "select weight_offset, additional_weight_offset from master_couriers where id=%s;", (cost_obj[5],)
            )
            ...
            if courier_data[0] != 0 and courier_data[1] != 0:
                ...
                if charged_weight > courier_data[0]:
                    forward_charge = (
                        charge_rate
                        + math.ceil((charged_weight - courier_data[0] * 1.0) / courier_data[1]) * per_step_charge
                    )
            else:
...
def accept_discrepency(resp):
    ...
    try:
        ...
        cur.execute(
            """UPDATE weight_discrepency SET status_id=2 WHERE shipment_id in
                        (SELECT id FROM shipments WHERE order_id in %s)"""
            % order_tuple_str
        )
...
def raise_dispute(resp):
    ...
    try:
        ...
        for i in range(1, 6):
            ...
            if file:
                file_url = process_upload_logo_file(
                    client_prefix,
                    file,
                    bucket="wareiqreconciliation",
                    file_name=str(order_id) + "_file" + str(i),
                    master_bucket="wareiqreconciliation",
                )
        ...
        if auth_data["user_group"] == "client":
            query_to_execute = query_to_execute.replace(
                "__CLIENT_FILTER__", "AND aa.client_prefix = '%s'" % client_prefix
            )
        if auth_data["user_group"] == "multi-vendor":
            cur.execute("SELECT vendor_list FROM multi_vendor WHERE client_prefix='%s';" % client_prefix)
            vendor_list = cur.fetchone()[0]
...
def get_passbook(resp):
    ...
    try:
        ...
        passbook_qs = db.session.query(WalletPassbook).filter(
            WalletPassbook.client_prefix == auth_data.get("client_prefix")
        )
        if filters.get("categories"):
            passbook_qs = passbook_qs.filter(WalletPassbook.category.in_(filters.get("categories")))
        if search:
            passbook_qs = passbook_qs.filter(
                or_(WalletPassbook.descr.contains(search), WalletPassbook.ref_no.contains(search))
            )
...
def get_delivery_zone(cur_2, pick_pincode, del_pincode):
    ...
    cur_2.execute("SELECT zone_value from city_zone_mapping where zone=%s and city=%s;", zone_select_tuple)
...
zone_step_charge_column_mapping = {"A": "a_step", "B": "b_step", "C": "c_step", "D": "d_step", "E": "e_step"}

...
from .queries import (
    ...
    select_wallet_deductions_query,
    select_wallet_remittance_query,
    ...
    def get(self, ver_type):
        try:
            ...
            if order_id:
                ...
                client_name = (
                    db.session.query(ClientMapping).filter(ClientMapping.client_prefix == order.client_prefix).first()
                )
                if client_name:
                ...
                if ver_type == "cod":
                    gather_prompt_text = (
                        "Hello %s, You recently placed an order from %s with amount %s."
                        " Press 1 to confirm your order or, 0 to cancel."
                        % (order.customer_name, client_name, str(order.payments[0].amount))
                    )

                    repeat_prompt_text = (
                        "It seems that you have not provided any input, please try again. Order from %s, "
                        "Order ID %s. Press 1 to confirm your order or, 0 to cancel."
                        % (client_name, order.channel_order_id)
                    )
                elif ver_type == "ndr":
                    gather_prompt_text = (
                        "Hello %s, delivery for your order from %s with amount %s was attempted today "
                        "but could not be delivered."
                        " Press 0 to re-attempt delivery or, press 1 to cancel order."
                        % (order.customer_name, client_name, str(order.payments[0].amount))
                    )
                ...
                response = {
                    ...
                    "max_input_digits": 1,
                    "repeat_menu": 2,
                    "repeat_gather_prompt": {"text": repeat_prompt_text},
                }
                return response, 200
            else:
                return {"success": False, "msg": "Order not found"}, 400
...
def verification_passthru(type):
    try:
        ...
        if (
            request.user_agent.browser == "safari"
            and request.user_agent.platform == "iphone"
            and request.user_agent.version == "13.0.1"
        ):
        ...
        if order_id:
            ...
            if type == "cod":
                ...
                ndr_obj = (
                    ...
                    .filter(NDRShipments.order_id == cod_ver.order_id)
                    .order_by(NDRShipments.date_created.desc())
                    .first()
                )
                if ndr_obj:
                    ndr_obj.current_status = (
                        "reattempt" if verified_via == "text" or cod_verified == False else "cancelled"
                    )
...
def get_downloads(resp):
    ...
    query_to_run = query_to_run.replace(
        "__PAGINATION__", "OFFSET %s LIMIT %s" % (str((page - 1) * per_page), str(per_page))
    )
...
def capture_payment(resp):
    ...
    recharge_obj = (
        db.session.query(ClientRecharges)
        .filter(
            ClientRecharges.transaction_id == razorpay_order_id,
            ClientRecharges.client_prefix == auth_data.get("client_prefix"),
        )
        .first()
    )
    ...
    mapping_obj = (
        db.session.query(ClientMapping).filter(ClientMapping.client_prefix == auth_data.get("client_prefix")).first()
    )
    if mapping_obj:
        mapping_obj.current_balance = mapping_obj.current_balance + recharge_obj.recharge_amount
        try:
            passbook_obj = WalletPassbook(
                client_prefix=mapping_obj.client_prefix,
            ...
            logger.error(
                "wallet recharge passbook failed: " + str(mapping_obj.client_prefix) + "\nError: " + str(e.args[0])
            )
...
def thirdwatch_webhook():
    try:
        ...
        client = (
            ...
            .filter(ClientMapping.unique_parameter == webhook_body["merchant_identifier"])
            .first()
        )
        if not client:
        ...
        order = (
            ...
            .filter(
                Orders.client_prefix == client.client_prefix, Orders.client_channel_id == str(order_data["order_id"])
            )
            .first()
        )

        if event.lower() == "score":
            thirdwatch_obj = ThirdwatchData(
                order=order,
...
def thirdwatch_webhook_partner():
    try:
        webhook_body = json.loads(request.data)
        thirdwatch_obj = ThirdwatchData(
            order_id=int(webhook_body["order_id"]),
...
def getshiporders(resp):
    try:
        ...
        courier_list = list()
        cur.execute(
            """select aa.courier_name from master_couriers aa
                        ...
                        order by bb.priority nulls last""".replace(
                "__CLIENT_PREFIX__", auth_data.get("client_prefix")
            )
        )
...
def website_case_study():
    data = [
        {
            "image_link": "https://wareiqfiles.s3.amazonaws.com/kamaayurveda.png",
...
def get_freshdesk_url(auth_data):
    ...
    req_url = "%s/login/sso?name=%s&email=%s&timestamp=%s&company=%s&hash=%s&redirect_to=%s" % (
        portal,
        name,
        email,
        timestamp,
        company,
        fd_hash,
        "https://wareiq.freshdesk.com/support/home",
    )
    return req_url
...
def freshdesk_url_new(auth_data):
    ...
    jwt_string = jwt.encode(
        payload,
        """MIICXgIBAAKBgQDLPqlfK8X7nnTOUzeoqpB/ITv0BBWtGJfPBEHMs4hqj6f4QQWJ
...
def ping_dev():
    ...
    cur = conn.cursor()
    cur.execute(
        """select order_id_channel_unique, awb, courier_name, dd.api_key, dd.unique_parameter from orders aa
    ...
    for order in all_orders:
        try:
            post_url = (
                "https://api.easyecom.io/Carrier/assignAWB?api_token=%s"
                % "073ed5f1decdc0529cf3fbf15a843f02795dbc5feb6761c2b3d69cb65760bd59"
            )
            post_body = {
                "invoiceId": order[0],
    ...
    cur = conn.cursor()
    cur.execute(
        """select aa.order_id_channel_unique, bb.api_key, cc.awb, aa.id from orders aa
    ...
    for order in all_orders:
        try:
            order_details = requests.get(
                "https://api.easyecom.io/orders/V2/getAllOrders?api_token=%s&invoice_id=%s" % (order[1], order[0])
            )
            if order_details.status_code == 200:
                order_details = order_details.json()
                if order_details["data"]["orders"]:
                    if (
                        order_details["data"]["orders"][0]["awb_number"]
                        and order_details["data"]["orders"][0]["awb_number"] != order[2]
                    ):
                        cur.execute(
                            "UPDATE shipments SET awb=%s WHERE order_id=%s",
                            (order_details["data"]["orders"][0]["awb_number"], order[3]),
                        )
    ...
    cur = conn.cursor()
    cur.execute(
        """select bb.api_key, aa.order_id_channel_unique, ee.courier_name, dd.awb, bb.unique_parameter from orders aa
                    left join client_channel bb on aa.client_channel_id=bb.id
                    left join (select * from order_status where status='Received') cc on cc.order_id=aa.id
                    left join shipments dd on dd.order_id=aa.id
                    left join master_couriers ee on dd.courier_id=ee.id
                    where bb.channel_id=7
                    and cc.status_time<'2021-06-04' and cc.status_time>'2021-06-03 19:00:00'"""
    )
    all_orders = cur.fetchall()
    for order in all_orders:
        try:
            post_url = "https://api.easyecom.io/Carrier/assignAWB?api_token=%s" % order[0]
            post_body = {
                "invoiceId": order[1],
                "api_token": order[0],
                "courier": order[2],
    ...
    for row in iter_rw:
        try:
            ...
            """
            sku_list.append(
                {"sku": sku, "warehouse": "TNPMRO", "quantity": del_qty, "type": "subtract", "remark": "21 feb sale"}
            )
            ...
            if row[0] % 100 == 0:
                headers = {
                    ...
                    + "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJleHAiOjE2MTU1NzI1MzUsImlhdCI6MTYxMjk4MDUzNSwic3ViIjo5fQ.H4MjEuE2zhPqSufcyC9WIr27coPALTn7IIlmgbtCnxI",
                    "Content-Type": "application/json",
                }

                data = {"sku_list": sku_list}
                req = requests.post(
                    "https://track.wareiq.com/products/v1/update_inventory", headers=headers, data=json.dumps(data)
                )
    ...
    data = {"sku_list": sku_list}
    req = requests.post("https://track.wareiq.com/products/v1/update_inventory", headers=headers, data=json.dumps(data))
    ...
    exotel_idx = 0
    exotel_sms_data = {"From": "01141182252"}
    ...
    for row in iter_rw:
        ...
        exotel_sms_data[sms_body_key] = str(row[1].SMS) + ". Thanks!"
        exotel_idx += 1
    ...
    while count == 250:
        create_fulfillment_url = (
            "https://f2e810c7035e1653f0191cb8f5da58f6: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@rattleandco.myshopify.com/admin/api/2020-07/products.json?limit=250&since_id=%s"
            % since_id
        )
    ...
    auth_session = OAuth1Session(
        "ck_43f358286bc3a3a30ffd00e22d2282db07ed7f5d", client_secret="cs_970ec6a2707c17fc2d04cc70e87972faf3c98918"
    )
    ...
    cur.execute(
        """select master_sku, GREATEST(available_quantity, 0) as available_quantity from
    ...
    for quan in all_quan:
        source_items.append({"sku": quan[0], "source_code": "default", "quantity": quan[1], "status": 1})

    magento_url = "https://www.kamaayurveda.com/rest/default/V1/inventory/source-items"
    body = {"sourceItems": source_items}
    headers = {"Authorization": "Bearer q4ldi2wasczvm7l8caeyozgkxf4qanfr", "Content-Type": "application/json"}
    r = requests.post(magento_url, headers=headers, data=json.dumps(body))

    return 0
    import requests, json

    magento_orders_url = """%s/V1/orders/12326""" % ("https://magento.feelmighty.com/rest")
    headers = {"Authorization": "Bearer " + "f3ekur9ci3gc0cb63y743dvcy3ptyxe5", "Content-Type": "application/json"}
    ...
    req = requests.get(
        "https://api.ecomexpress.in/apiv2/fetch_awb/?username=warelqlogisticspvtltd144004_pro&password=LdGvdcTFv6n4jGMT&count=10000&type=PPD"
    )

    from requests_oauthlib import OAuth1Session

    auth_session = OAuth1Session(
        "ck_cd462226a5d5c21c5936c7f75e1afca25b9853a6", client_secret="cs_c897bf3e770e15f518cba5c619b32671b7cc527c"
    )
    ...
    req = requests.get(
        "https://plapi.ecomexpress.in/track_me/api/mawbd/?awb=8636140444,8636140446,8636140435&username=warelqlogisticspvtltd144004_pro&password=LdGvdcTFv6n4jGMT"
    )
    req = json.loads(json.dumps(xmltodict.parse(req.content)))
    cancel_body = json.dumps(
        {"AWBNumber": "14201720011569", "XBkey": "NJlG1ISTUa2017XzrCG6OoJng", "RTOReason": "Cancelled by seller"}
    )
    headers = {"Authorization": "Basic " + "NJlG1ISTUa2017XzrCG6OoJng", "Content-Type": "application/json"}
    req_can = requests.post(
        "http://xbclientapi.xbees.in/POSTShipmentService.svc/RTONotifyShipment", headers=headers, data=cancel_body
    )
    ...
    for row in iter_rw:
        try:
            ...
            pickup_point = PickupPoints(
                pickup_location=str(row[1].SellerName),
            ...
            return_point = ReturnPoints(
                return_location=str(row[1].SellerName),
                name=str(row[1].SellerName),
                phone=str(row[1].Contact),
                address=str(row[1].SellerAddress),
                address_two="",
            ...
            client_pickup = ClientPickups(pickup=pickup_point, return_point=return_point, client_prefix="FURTADOS")
    ...
    auth_session = OAuth1Session(
        "ck_e6e779af2808ee872ba3fa4c0eab26b5f434af8c", client_secret="cs_696656b398cc783073b281eca3bf02c3c4de0cd1"
    )
    url = (
        "%s/wp-json/wc/v3/orders?per_page=100&order=asc&consumer_key=ck_e6e779af2808ee872ba3fa4c0eab26b5f434af8c&consumer_secret=cs_696656b398cc783073b281eca3bf02c3c4de0cd1"
        % ("https://silktree.in")
    )
    ...
    auth_session = OAuth1Session(
        "ck_9a540daf59bd7e78268d80ed0db14d03a0e68b57", client_secret="cs_017c6ac59089de2dfd4e2f99e56513aec093464"
    )
    url = (
        "%s/wp-json/wc/v3/products?per_page=100&consumer_key=ck_9a540daf59bd7e78268d80ed0db14d03a0e68b57&consumer_secret=cs_017c6ac59089de2dfd4e2f99e56513aec093464"
    ...
    for order_id in order_ids:
        ...
        order_qs = (
            db.session.query(Orders)
            .filter(Orders.order_id_channel_unique == order_id, Orders.client_prefix == "KAMAAYURVEDA")
            .all()
        )
        ...
        cur.execute(
            """UPDATE op_association SET order_id=%s where order_id=%s""" % (str(keep_order.id), str(del_order.id))
        )
        keep_order.payments[0].amount = keep_order.payments[0].amount + del_order.payments[0].amount
        keep_order.payments[0].shipping_charges = (
            keep_order.payments[0].shipping_charges + del_order.payments[0].shipping_charges
        )
    ...
    for row in iter_rw:
        try:
            ...
            prod_obj_x = (
                ...
                .filter(Products.master_sku == sku, Products.client_prefix == "KAMAAYURVEDA")
                .first()
            )
            if not prod_obj_x:
    ...
    magento_orders_url = (
        """%s/V1/orders?searchCriteria[filter_groups][0][filters][0][field]=updated_at&searchCriteria[filter_groups][0][filters][0][value]=%s&searchCriteria[filter_groups][0][filters][0][condition_type]=gt"""
        % ("https://demokama2.com/rest/default", "2020-06-23")
    )
    headers = {"Authorization": "Bearer " + "gfl1ilzw8iwe4yf06iuiophjfq1gb49k", "Content-Type": "application/json"}
    ...
    all_orders = (
        db.session.query(Orders)
        .filter(
            Orders.client_prefix == "SSTELECOM", Orders.status.in_(["IN TRANSIT"]), Orders.channel_order_id > "2093"
        )
        .all()
    )
    for order in all_orders:
        shopify_url = (
            "https://e156f178d7a211b66ae0870942ff32b1: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@trendy-things-2020.myshopify.com/admin/api/2020-04/orders/%s/fulfillments.json"
        ...
        req_ful = requests.post(shopify_url, data=json.dumps(fulfil_data), headers=ful_header)
    return 0

    return 0
    headers = {"Content-Type": "application/json", "XBKey": "NJlG1ISTUa2017XzrCG6OoJng"}
    ...
    headers = {"Authorization": "Bearer h5e9tmzud0c8p0o82gaobegxpw9tjaqq", "Content-Type": "application/json"}
    apiuser = "wareiq"
    ...
    all_ord = (
        db.session.query(Orders)
        .filter(Orders.client_prefix == "ZLADE")
        .filter(Orders.status.in_(["DELIVERED", "IN TRANSIT", "READY TO SHIP"]))
        .all()
    )
    for ord in all_ord:
    ...
    for prod in r.json():
        try:
            ...
            if prod["dimensions"]["length"]:
                dimensions = {
                    "length": int(prod["dimensions"]["length"]),
            ...
            for idx, value in enumerate(prod["variations"]):
                ...
                prod_obj_x = Products(
                    name=prod["name"] + " - " + prod["attributes"][0]["options"][idx],
                    sku=str(value),
                    master_sku=prod["sku"],
                    dimensions=dimensions,
                    weight=weight,
    ...
    for order in orders:
        if order.shipments and order.shipments[0].awb:
            create_fulfillment_url = (
                "https://%s:%s@%s/admin/api/2019-10/orders.json?limit=100&since_id=1980049653824"
                % ("a97f8f4744d02183b84b20469af2bc3d", "f9ed6255a50a66a7af6bcdff93b3ce81", "unitedbyhope.myshopify.com")
            )
    ...
    auth_session = OAuth1Session(
        "ck_48b2a03de2cc5906951ff783c6c0cf83d0fa6af4", client_secret="cs_5d401d8aeaa8a16f5f82c81089290b392420891d"
    )
    ...
    for prod in data["products"]:
        for prod_obj in prod["variants"]:
            prod_obj_x = Products(
                name=prod["title"] + " - " + prod_obj["title"],
                sku=str(prod_obj["id"]),
                master_sku=prod_obj["sku"],
                dimensions={"length": 7, "breadth": 14, "height": 21},
                weight=0.35,
                price=float(prod_obj["price"]),
    ...
    for prod in data["products"]:
        for e_sku in prod["variants"]:
            try:
                ...
                prod_obj = (
                    db.session.query(Products)
                    .join(ProductQuantity, Products.id == ProductQuantity.product_id)
                    .filter(Products.sku == str(e_sku["id"]))
                    .first()
                )
    ...
    prod_list = requests.get(
        "https://640e8be5fbd672844636885fc3f02d6b:07d941b140370c8c975d8e83ee13e524@clean-canvass.myshopify.com/admin/api/2019-10/products.json?limit=250"
    ).json()
    ...
    if not myfile:
        prod_obj = (
            db.session.query(Products)
            .join(ProductQuantity, Products.id == ProductQuantity.product_id)
            .filter(Products.sku == str(row[1]["sku"]))
            .first()
        )
    ...
    all_orders = (
        db.session.query(Orders)
        .filter(Orders.status.in_(["IN TRANSIT", "PENDING", "DISPATCHED"]))
        .filter(Orders.client_prefix == "DAPR")
        .all()
    )

    headers = {"Content-Type": "application/json", "Authorization": "Token 1368a2c7e666aeb44068c2cd17d2d2c0e9223d37"}
    for order in all_orders:
        try:
            shipment_body = {"waybill": order.shipments[0].awb, "phone": order.customer_phone}
            req = requests.post(
                "https://track.delhivery.com/api/p/edit", headers=headers, data=json.dumps(shipment_body)
            )
    ...
    for row in data_xlsx.iterrows():
        ...
        if not prod_obj:
            prod_obj = Products(
                name=str(row[1]["sku"]),
                sku=str(row[1]["sku"]),
                dimensions={"length": 3, "breadth": 26, "height": 27},
                weight=0.5,
                price=0,
                client_prefix="LMDOT",
                active=True,
    ...
    for location in pick:
        ...
        headers = {
            "Authorization": "Token c5fd3514bd4cb65432ce31688b049ca6cf417b28",
            "Content-Type": "application/json",
        }
        ...
        headers = {
            "Authorization": "Token 538ee2e5f226a85e4a97ad3aa0ae097b41bdb89c",
            "Content-Type": "application/json",
        }
        req = requests.post(delhivery_url, headers=headers, data=json.dumps(loc_body))
    ...
    fulfie_data = {
        "fulfillment": {
            ...
            "tracking_urls": ["https://www.delhivery.com/track/package/3991610025771"],
    ...
    requests.post(url, data=json.dumps(fulfie_data), headers=ful_header)
    import requests
    ...
    data_xlsx = pd.read_excel(myfile)
    headers = {"Authorization": "Token 5f4c836289121eaabc9484a3a46286290c70e69e", "Content-Type": "application/json"}
    ...
    order_qs = (
        db.session.query(Orders)
        .filter(Orders.client_prefix == "KYORIGIN")
        .filter(Orders.status.in_(["READY TO SHIP", "PICKUP REQUESTED", "NOT PICKED"]))
        .all()
    )
    for order in order_qs:
        try:
    ...
    orders = (
        ...
        .filter(Orders.status == "IN TRANSIT", Orders.status_type == "UD", Orders.client_prefix == "KYORIGIN")
        .all()
    )
    awb_str = ""
    ...
    req = requests.get(
        "https://track.delhivery.com/api/status/packages/json/?waybill=%s&token=1368a2c7e666aeb44068c2cd17d2d2c0e9223d37"
        % awb_str
    ).json()
    ...
    for shipment in req["ShipmentData"]:
        try:
            ...
            exotel_sms_data[sms_body_key] = (
                "Dear Customer, your Know Your Origin order with AWB number %s is IN-TRANSIT via Delhivery and will be delivered by %s. Thank you for ordering."
    ...
    for point in data:
        try:
            ...
            courier = (
                db.session.query(MasterCouriers)
                .filter(MasterCouriers.courier_name == point["shipments"][0]["courier"])
                .first()
            )
            shipment.courier = courier

            payment = OrdersPayments(
                payment_mode=point["payment_method"], amount=float(point["total"]), currency="INR", order=new_order
            )
...
def send_dev():
    ...
    cur.execute(
        """select bb.order_id_channel_unique, aa.awb from shipments aa
                        left join orders bb
    ...
    for order in all_orders:
        create_fulfillment_url = "https://%s:%s@%s/admin/api/2019-10/orders/%s/fulfillments.json" % (
            "dc8ae0b7f5c1c6558f551d81e1352bcd",
            "00dfeaf8f77b199597e360aa4a50a168",
            "origin-clothing-india.myshopify.com",
            order[0],
        )
    ...
    cur.execute(
        """select aa.order_id, bb.customer_phone from cod_verification aa
                        left join orders bb on aa.order_id=bb.id
    ...
    for idx, awb in enumerate(awbs):
        ...
        sms_data[sms_body_key] = (
            "Dear Customer, we apologise for the delay. Your order from Know Your Origin has not been shipped due to huge volumes. It will be shipped with in next two days with AWB no. %s by Delhivery."
            % str(awb[0])
        )
        itt += 1
    lad = requests.post(

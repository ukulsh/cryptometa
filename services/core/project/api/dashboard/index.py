    ...
    try:
        ...
        if auth_data["user_group"] == "client":
            query_to_run = query_to_run.replace(
                "__CLIENT_FILTER__", "AND aa.client_prefix='%s'" % auth_data["client_prefix"]
            )
        elif all_vendors:
            query_to_run = query_to_run.replace(
...
def get_dashboard_verification(resp):
    ...
    try:
        ...
        if auth_data["user_group"] == "client":
            query_to_run_cod = query_to_run_cod.replace(
                "__CLIENT_FILTER__", "AND bb.client_prefix='%s'" % auth_data["client_prefix"]
            )
            query_to_run_ndr = query_to_run_ndr.replace(
                "__CLIENT_FILTER__", "AND bb.client_prefix='%s'" % auth_data["client_prefix"]
            )
        elif all_vendors:
            query_to_run_cod = query_to_run_cod.replace(
                "__CLIENT_FILTER__", "AND bb.client_prefix in %s" % str(tuple(all_vendors))
            )
            query_to_run_ndr = query_to_run_ndr.replace(
                "__CLIENT_FILTER__", "AND bb.client_prefix in %s" % str(tuple(all_vendors))
            )
...
def get_dashboard_ndr(resp):
    ...
    try:
        ...
        query_to_run_count = """select status, count(count) from 
                            ...
                            and aa.date_created+interval '5.5 hours'<'%s'
                            __CLIENT_FILTER__
                            group by bb.id, bb.status) xx group by status""" % (
            from_date,
            to_date,
        )
        ...
        if auth_data["user_group"] == "client":
            query_to_run_count = query_to_run_count.replace(
                "__CLIENT_FILTER__", "AND bb.client_prefix='%s'" % auth_data["client_prefix"]
            )
            query_to_run_reason = query_to_run_reason.replace(
                "__CLIENT_FILTER__", "AND bb.client_prefix='%s'" % auth_data["client_prefix"]
            )
        elif all_vendors:
            query_to_run_count = query_to_run_count.replace(
...
def get_static_data(resp):
    ...
    try:
        ...
        query_to_run_count = """select oc.order_date as req_date, oc.count as orders, ROUND(oc.sum) as revenue, 
                                ...
                                on oc.order_date = dc.status_time""".replace(
            "__FROM_DATE__", date_yesterday
        )

...
from project.api.utilities.constants import (
    aws_access_key_id,
    aws_secret_access_key,
    aws_region,
    aws_client_data_bucket,
    aws_region_prefix,
)
...
def process_upload_logo_file(
    ...
    if file_name:
        file_key = "/%s/%s.%s" % (
            bucket,
            client_prefix + "_" + env + file_name,
            file_ext,
        )
    else:
        file_key = "/%s/%s.%s" % (
            bucket,
            client_prefix + "_" + env + "_logo_file",
            file_ext,
        )
    ...
    file_url = "https://%s/%s%s" % (
        aws_region,
        master_bucket if master_bucket else aws_client_data_bucket,
        urllib.parse.quote(file_key),
    )
    return file_url
...
def upload_to_s3(file, bucket_name, key, aws_region_host):
    ...
    conn = boto.connect_s3(
        aws_access_key_id, aws_secret_access_key, host=aws_region_host
    )
    ...
    sent = k.set_contents_from_file(
        file, cb=None, md5=None, reduced_redundancy=False, rewind=True
    )
